...
keeping it in the repository would mean the quickstart opens with a ``git
clone`` performed solely to obtain one file. Shipping it as package data and
exec'ing it here keeps ``uvx data-platform-mcp`` the only thing anyone installs.
"""
...
def run_setup(args: list[str]) -> int:
    ...
    # explicit bash invocation means the wheel need not preserve mode bits.
